...
const navItems = [
  ...
  { href: "/admin/form", label: "Forms", icon: FileText },
];
      ...
      <ul className="flex items-center justify-center space-x-1 sm:space-x-2">
        {pathname !== "/admin" && (
          <li>
            ...
            >
              <LayoutDashboard className="h-4 w-4 sm:h-5 sm:w-5 text-muted-foreground group-hover:text-accent-foreground" />
              <span className="hidden sm:inline">Back to User Dashboard</span>
              <span className="sm:hidden text-xs">User Dash</span>
            </Link>
          </li>
        )}
        {navItems.map((item) => {
